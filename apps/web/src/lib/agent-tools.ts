...
const AGENT_TOOLS_RAW: AgentTool[] = [
  ...
  // ─── RH RWA Phase 6 — Agent Skills ─────────────────────────────────────
  {
    ...
    category: "portfolio",
    inputs: [
      { key: "wallet",        label: "Wallet",           placeholder: "0x…", required: true },
      { key: "ticker",        label: "Ticker",           placeholder: "AAPL", required: true },
      { key: "amount_usd",    label: "Amount USD/period",placeholder: "50",   required: true },
      { key: "cadence",       label: "Cadence",          placeholder: "hour / day / week" },
      { key: "total_periods", label: "Total periods",    placeholder: "12" },
      { key: "denom",         label: "Denom",            placeholder: "USDG or WETH" },
      { key: "slippage_bps",  label: "Slippage bps",     placeholder: "100" },
      { key: "persist",       label: "Persist to KV",    placeholder: "true or false" },
    ],
    isComposite: false,
    price: "$0.20", priceUSDC: 200000,
    x402Url: `${X402_BASE}/rh-rwa-dca`,
    x402Body: (v) => ({
      wallet: v.wallet ?? "",
      ticker: v.ticker ?? "",
      amount_usd: v.amount_usd ? Number(v.amount_usd) : 0,
      cadence: v.cadence ?? "week",
      total_periods: v.total_periods ? Number(v.total_periods) : 12,
      denom: v.denom ?? "USDG",
      slippage_bps: v.slippage_bps ? Number(v.slippage_bps) : 100,
  ...
  },
  {
    id: "rh-stock-alert",
    name: "RH Stock Alert",
    description: "Register a Chainlink-price threshold alert for a RH RWA. Polls Chainlink once at registration for immediate met/pending status; optionally persists to KV for a poller to watch. Above / below direction, USD threshold.",
    agentHandle: "blueagent", agentName: "Blue Agent", agentType: "blue",
    category: "signal",
    inputs: [
      { key: "ticker",        label: "Ticker",           placeholder: "AAPL", required: true },
      { key: "threshold_usd", label: "Threshold USD",    placeholder: "350",   required: true },
      { key: "direction",     label: "Direction",        placeholder: "above or below" },
      { key: "recipient",     label: "Recipient (opt.)", placeholder: "0x… or channel ID" },
      { key: "webhook_url",   label: "Webhook (opt.)",   placeholder: "https://…" },
      { key: "persist",       label: "Persist to KV",    placeholder: "true or false" },
      { key: "ttl_hours",     label: "TTL hours",        placeholder: "168" },
    ],
    isComposite: false,
    price: "$0.10", priceUSDC: 100000,
    x402Url: `${X402_BASE}/rh-stock-alert`,
    x402Body: (v) => ({
      ticker: v.ticker ?? "",
      threshold_usd: v.threshold_usd ? Number(v.threshold_usd) : 0,
      direction: v.direction ?? "above",
      recipient: v.recipient ?? "",
      webhook_url: v.webhook_url ?? "",
      persist: v.persist === "true",
      ttl_hours: v.ttl_hours ? Number(v.ttl_hours) : 168,
    }),
  },
  {
    id: "rh-stock-report",
    ...
    inputs: [
      { key: "ticker",  label: "Ticker",         placeholder: "AAPL", required: true },
      { key: "horizon", label: "News horizon",   placeholder: "week / month" },
    ],
    isComposite: false,
    price: "$0.20", priceUSDC: 200000,
    x402Url: `${X402_BASE}/rh-stock-report`,
    x402Body: (v) => ({ ticker: v.ticker ?? "", horizon: v.horizon ?? "week" }),
  },
  {
